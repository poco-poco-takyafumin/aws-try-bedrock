...
#
# 注意: これはBedrockサービス単体ではなく「AWSアカウント全体」の請求額に対するアラーム。
# 本来はユースケース単位ではなくアカウント共通の設定であり、複数ユースケースを
# 同一アカウントで運用する場合は重複作成を避けて1つだけ持つべきリソース
# （docs/00の未決事項「アカウント分離するか」に関連）。他ユースケース(02, 03)の
# Terraformでも同様のリソースを作らないよう注意すること。
#
